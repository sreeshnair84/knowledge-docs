...
def fix_image_references(md_text, img_dir, img_prefix, base_name):
    ...
    auto_pattern = re.compile(r"!\[([^\]]*)\]\((" + img_dir_esc + r"[/\\]([^)]+\.png))\)")

    renamed = {}
    counter = [0]

    def rename_match(m):
        alt, _old_path, fname = m.group(1), m.group(2), m.group(3)
...
def strip_running_headers_footers(md_text, min_pages=3):
    ...
    repeated_code = {norm for norm, cnt in code_content_counts.items() if cnt >= min_pages}
    ...
    to_strip = {norm for norm, count in line_page_count.items() if count >= min_pages}
...
def main():
    ap = argparse.ArgumentParser(description="Convert PDF to Markdown using pymupdf4llm (ML layout detection).")
