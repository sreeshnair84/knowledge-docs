---
title: "IBM Certified Developer — Quantum Computation Study Guide"
...
exam_code: "C1000-171"
exam_validity: "2026"
covers_version: "Qiskit v1.x / Runtime v2"
---

# IBM Certified Developer — Quantum Computation Study Guide
...
|---|---|
| Exam code | C1000-171 |
| Credential | IBM Certified Developer — Quantum Computation |
| Format | Multiple choice |
| Questions | 70 |
| Duration | 90 minutes |
| Passing score | 49/70 (70%) |
| Prerequisites | C1000-112 (Associate level) or equivalent knowledge |
| Qiskit version | v1.x / Qiskit Runtime v2 |

Builds on Associate level: adds Runtime v2, VQE, QAOA, QML, noise models, and hybrid architecture design. Every practice session should end with a working, end-to-end quantum program.
...
2. [Qiskit Runtime v2 — Sampler, Estimator, Sessions](#section-2-qiskit-runtime-v2--sampler-estimator-sessions)
3. [Advanced Circuit Techniques](#section-3-advanced-circuit-techniques)
4. [Noise Models & Error Mitigation](#section-4-noise-models--error-mitigation)
5. [Variational Algorithms — VQE & QAOA](#section-5-variational-algorithms--vqe--qaoa)
6. [Quantum Machine Learning — QNNs, Kernels, PennyLane](#section-6-quantum-machine-learning--qnns-kernels-pennylane)
7. [Quantum Circuit Library](#section-7-quantum-circuit-library)
8. [Advanced Transpilation — Custom PassManagers](#section-8-advanced-transpilation--custom-passmanagers)
9. [Quantum Information — Fidelity, Entanglement, Tomography](#section-9-quantum-information--fidelity-entanglement-tomography)
10. [Hybrid Architecture Patterns](#section-10-hybrid-architecture-patterns)
11. [Practice Exam — 15 Questions with Explanations](#section-11-practice-exam--15-questions-with-explanations)
12. [Master Reference & Resources](#section-12-master-reference--resources)
...
|---|---|---|
| Advanced Circuit Construction | ~20% | Parameterised circuits, circuit library, pulse-level, dynamical decoupling |
...
sampler_hw = Sampler(backend, options=options)
tqc_hw = transpile(qc, backend, optimization_level=3)
job_hw = sampler_hw.run([tqc_hw], shots=8192)
print(job_hw.job_id())  # save for retrieval
```
...
## Section 3: Advanced Circuit Techniques
...
## Section 4: Noise Models & Error Mitigation
...
t1, t2    = 80e-6, 120e-6   # T1=80µs, T2=120µs (typical IBM)
...
ZNE: run circuit at scaled noise levels, extrapolate to zero noise. Most practical error mitigation for NISQ devices.
...
# Gate folding replaces G with G*G†*G (3x noise), etc.
noise_levels = [1, 2, 3]
expectation_vals = [execute(qc) for _ in noise_levels]
...
## Section 5: Variational Algorithms — VQE & QAOA
...
# Define Hamiltonian: H2 molecule (simplified, Jordan-Wigner mapping)
...
# Define ansatz — EfficientSU2: hardware-efficient, good for NISQ
...
# Optimise — COBYLA: gradient-free, works well with noisy circuits
...
# Quantum Kernel: K(x,x') = |<phi(x)|phi(x')>|^2
...
# Compute kernel matrix (O(n^2) circuit evaluations)
...
> **Exam Focus:** Know that `FidelityQuantumKernel` computes kernel entries as circuit FIDELITY (overlap squared). The quantum advantage requires that the feature space is classically hard to compute — `ZZFeatureMap` achieves this via entangling layers.

---

## Section 7: Quantum Circuit Library
...
# TwoLocal — more flexible than EfficientSU2
two_local = TwoLocal(
    num_qubits=4,
...
# RealAmplitudes — only real amplitudes, no imaginary parts
...
> **Developer Tip:** Always use circuit library ansatze (`EfficientSU2`, `TwoLocal`) rather than building your own. The exam will test your knowledge of their parameters: `reps`, `entanglement`, `num_parameters`.

---

## Section 8: Advanced Transpilation — Custom PassManagers
...
## Section 9: Quantum Information — Fidelity, Entanglement, Tomography
...
# Von Neumann entropy — measures entanglement: 0 = product state, 1 = maximally entangled
...
## Section 10: Hybrid Architecture Patterns
...
This is the TEMPLATE for all variational quantum algorithms — VQE, QAOA, and QNN training all follow this pattern.
...
class VariationalHybridLoop:
    ...
    def run(self, x0, method='COBYLA', maxiter=200):
        with Session(backend=self.backend) as session:
            ...
            return minimize(cost, x0, method=method,
...
| Decision | Guidance |
|---|---|
| **Problem Encoding** | Map domain problem to quantum Hamiltonian or oracle. Use Jordan-Wigner for fermionic systems; QUBO for combinatorial problems. Encoding is often the hardest step. |
| **Hardware vs Simulator** | Use simulators for development (free, fast). Switch to real hardware for final benchmarking. Use noise models to bridge the gap. |
| **Error Mitigation** | For NISQ: ZNE + Twirling is the standard combo. For critical accuracy: Probabilistic Error Cancellation (PEC). Always include readout calibration. |
| **Shot Budget** | Estimator: 1000–10000 shots per expectation value. Sampler: 4096–8192 for distribution accuracy. ZNE at 3 noise levels: multiply by 3. |
| **Classical Optimiser** | Low noise, few params: BFGS (gradient-based). High noise, many params: SPSA (noise-robust). Exploration needed: CMA-ES or Bayesian. |
| **Anti-Pattern** | Never put quantum execution in a real-time inference loop. QPU overhead (queue + execution + readout) is seconds to minutes. Design for batch or async. |

---

## Section 11: Practice Exam — 15 Questions with Explanations
...
For the full 70-question exam experience, complete IBM Quantum Learning's graded assessments at [learning.quantum.ibm.com](https://learning.quantum.ibm.com) — especially the 'Quantum Machine Learning' and 'Advanced Circuits' modules.
...
*Answer: **B** — ZNE: run at 1×, 2×, 3× noise (via gate folding), fit a polynomial, extrapolate to noise=0.*
...
*Answer: **B** — Fidelity = |⟨ψ|φ⟩|² = 1 iff states are identical (up to global phase).*
...
| `EfficientSU2` | `num_parameters` | `(reps+1) * n_qubits * 2` |
| `state_fidelity` | `(sv1, sv2)` | Returns \|⟨ψ\|φ⟩\|² |
...
- **IBM Quantum Learning:** [learning.quantum.ibm.com](https://learning.quantum.ibm.com) — complete ALL learning paths
- **Qiskit documentation:** [docs.quantum.ibm.com](https://docs.quantum.ibm.com)
- **qiskit-ibm-runtime docs:** [qiskit.github.io/qiskit-ibm-runtime](https://qiskit.github.io/qiskit-ibm-runtime)
- **PennyLane QML tutorials:** [pennylane.ai/qml](https://pennylane.ai/qml) (for QML section)
- **Exam registration:** [ibm.com/training/certification/C1000171](https://www.ibm.com/training/certification/C1000171)
...
*Cross-reference: [IBM Associate Cert Guide](./IBM_Associate_Quantum_CertGuide.md) (prerequisite) · [Quantum AI Zero to Mastery](./zero-to-mastery.md) (full 12-week programme)*
