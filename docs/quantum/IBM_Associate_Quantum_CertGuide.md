...
tags: ["quantum-computing", "qiskit", "certification"]
exam_code: "C1000-112"
exam_validity: "2026"
covers_version: "Qiskit v1.x"
---

# IBM Certified Associate Developer — Quantum Computation Study Guide
...
|---|---|
| Exam code | C1000-112 |
| Credential | IBM Certified Associate Developer — Quantum Computation |
| Format | Multiple choice |
| Questions | 60 |
| Duration | 90 minutes |
| Passing score | 43/60 (72%) |
| Qiskit version | v1.x |
| Question style | Scenario-based: read code and predict output, or choose the correct API call |

---

## Contents

1. [Exam Blueprint & Domain Weighting](#section-1-exam-blueprint--domain-weighting)
2. [Installation & Environment Setup](#section-2-installation--environment-setup)
3. [Quantum Circuits — QuantumCircuit API](#section-3-quantum-circuits--quantumcircuit-api)
4. [Gates — Single, Multi-Qubit & Parameterised](#section-4-gates--single-multi-qubit--parameterised)
5. [Measurements, Classical Registers & Conditionals](#section-5-measurements-classical-registers--conditionals)
6. [Simulators — AerSimulator & Statevector](#section-6-simulators--aerosimulator--statevector)
7. [Visualisation](#section-7-visualisation)
8. [Transpilation — PassManager & Optimisation Levels](#section-8-transpilation--passmanager--optimisation-levels)
9. [Real Hardware — IBM Quantum & Job Management](#section-9-real-hardware--ibm-quantum--job-management)
10. [Quantum Algorithms — Full Implementations](#section-10-quantum-algorithms--full-implementations)
11. [Practice Exam — 20 Questions with Explanations](#section-11-practice-exam--20-questions-with-explanations)
12. [Quick-Reference Cheat Sheet](#section-12-quick-reference-cheat-sheet)

---

## Section 1: Exam Blueprint & Domain Weighting
...
> **Exam Strategy:** 32% + 29% = 61% is circuits + algorithms. Master QuantumCircuit and the 4 algorithms cold. The remaining 39% (visualisation + backends) is straightforward API recall. Aim for 90%+ on circuits/algorithms; 80%+ elsewhere.
...
Required for running on real hardware:
...
## Section 4: Gates — Single, Multi-Qubit & Parameterised
...
## Section 6: Simulators — AerSimulator & Statevector
...
# AerSimulator (main exam simulator)
sim = AerSimulator()  # method='automatic' by default

# Statevector simulation (exact, no shots needed)
...
# Get statevector
qc_sv = qc.copy()
qc_sv.save_statevector()  # Aer-specific instruction
tqc = transpile(qc_sv, sv_sim)
job = sv_sim.run(tqc)
sv = job.result().get_statevector()
print(sv)  # [0.707, 0, 0, 0, 0, 0, 0, 0.707] -- amplitudes for |000> and |111>

# Shot-based simulation
qc_m = qc.copy()
qc_m.measure_all()
tqc_m = transpile(qc_m, sim)
job = sim.run(tqc_m, shots=8192)
counts = job.result().get_counts()  # ~4096 x '000', ~4096 x '111'
```
...
# Will show small probabilities for '001', '010', etc. (noise)
```

```python
# Statevector probabilities (no Aer needed)
from qiskit.quantum_info import Statevector

qc2 = QuantumCircuit(2)
qc2.h(0); qc2.cx(0, 1)
sv = Statevector(qc2)  # directly from circuit — no Aer required
print(sv.probabilities_dict())  # {'00': 0.5, '11': 0.5}
print(sv.probabilities())       # [0.5, 0.0, 0.0, 0.5]

# DensityMatrix
from qiskit.quantum_info import DensityMatrix
dm = DensityMatrix(qc2)
print(dm.purity())  # 1.0 for pure state
```
...
# Bloch sphere — shows qubit state as a point on the unit sphere
...
## Section 8: Transpilation — PassManager & Optimisation Levels
...
# Optimisation level 0: no optimisation (fastest compile)
tqc0 = transpile(qc, sim, optimization_level=0)
# Optimisation level 1: light optimisation (default)
tqc1 = transpile(qc, sim, optimization_level=1)
# Optimisation level 2: medium optimisation
tqc2 = transpile(qc, sim, optimization_level=2)
# Optimisation level 3: heavy optimisation (slowest compile, best circuit)
tqc3 = transpile(qc, sim, optimization_level=3)

print(f'Depth level 0: {tqc0.depth()}')
print(f'Depth level 3: {tqc3.depth()}')
...
```python
# PassManager for custom transpilation
...
optimised = pm.run(qc)
print(f'Custom PM depth: {optimised.depth()}')

# Check basis gates of a backend
# For AerSimulator, basis gates include: cx, u, measure, reset
# For IBM Eagle: ['ecr','id','rz','sx','x'] (varies by device)
```
...
## Section 9: Real Hardware — IBM Quantum & Job Management
...
# Get least busy backend with >= 5 qubits
...
print(config.n_qubits)      # 127
print(config.basis_gates)   # native gate set
print(config.coupling_map)  # qubit connectivity
...
```python
# Submit job with SamplerV2 (Qiskit Runtime v2)
qc = QuantumCircuit(2, 2)
qc.h(0); qc.cx(0, 1); qc.measure([0, 1], [0, 1])
...
pub_result = result[0]
counts = pub_result.data.meas.get_counts()  # {'00': ~2048, '11': ~2048}

# Retrieve existing job by ID
retrieved_job = service.job('YOUR_JOB_ID_HERE')
retrieved_result = retrieved_job.result()

# Cancel a pending job
job.cancel()  # only works if job is still QUEUED
```
...
## Section 10: Quantum Algorithms — Full Implementations

### Algorithm 1: Deutsch-Jozsa

Determines if f:{0,1}^n → {0,1} is **CONSTANT** or **BALANCED** in 1 query. Classical worst case: 2^(n-1)+1 queries.
...
def deutsch_jozsa(n, oracle_type='balanced'):
    ...
    if oracle_type == 'constant_0':
        ...
        for i in range(n):    # CNOT from each input qubit to ancilla
            qc.cx(i, n)
    qc.barrier()
...
Finds a marked element in unsorted database of N items. Speedup: O(√N) vs O(N) classical.
...
Transmit unknown qubit state |ψ⟩ using 2 classical bits + 1 entangled pair. Demonstrates entanglement as a resource.
...
QFT is the quantum analogue of the DFT. Runs in O(n²) vs O(n·2ⁿ) classically. Foundation of Shor's algorithm and phase estimation.
...
> **Exam Tip:** You will not be asked to derive QFT from scratch. You WILL be asked: What is the gate complexity? O(n²). What is QFT used for? Phase estimation, Shor's algorithm. What does it do? Maps computational basis to Fourier basis.

---

## Section 11: Practice Exam — 20 Questions with Explanations

Work through these questions under exam conditions: 30 minutes for 20 questions. Then review every explanation — especially questions you got right by guessing.

For the full 60-question exam experience, complete IBM Quantum Learning's official graded assessments at [learning.quantum.ibm.com](https://learning.quantum.ibm.com).
...
*Answer: **B** — H creates superposition on q0, CNOT entangles. Result is Bell state Φ+.*
...
B) qubit 0 → cbit 1, qubit 1 → cbit 0  
C) cbit 0 → qubit 1, cbit 1 → qubit 0  
...
A) `AerSimulator(method='qasm')`  
B) `AerSimulator(method='statevector')`  
...
*Answer: **C** — Rightmost bit = q[0]. `'01'` → q0=1; `'10'` → q0=0. So q0 is 50/50.*
...
A) Runs the circuit on the backend  
B) Converts the circuit to backend's native gates with maximum optimisation  
C) Simulates with 3 rounds of error correction  
D) Applies 3 transpilation passes only  

*Answer: **B** — `optimization_level` controls gate merging/cancellation aggressiveness (0–3).*

---

**Q9.** The Toffoli gate (CCX) flips the target qubit when:

A) Either control qubit is |1⟩  
B) The first control qubit is |1⟩  
C) Both control qubits are |1⟩  
D) The target qubit is |0⟩  

*Answer: **C** — CCX = AND gate. Flips target only when BOTH controls are |1⟩.*

---

**Q10.** `qc.barrier()` primarily serves to:

A) Measure all qubits simultaneously  
B) Reset all qubits to |0⟩  
C) Prevent gate reordering during transpilation and visually separate sections  
D) Add a time delay in the circuit  

*Answer: **C** — Barriers are optimisation fences + visual separators. No physical operation.*

---

**Q11.** What is the output of `qc.count_ops()` for a circuit with 2 H gates and 1 CX?
...
*Answer: **B** — `count_ops()` returns an OrderedDict mapping gate name to count.*
...
*Answer: **B** — Grover: O(√N) queries. Classical unstructured search: O(N).*
...
*Answer: **C** — H² = I. X, Y, Z, CNOT are also self-inverse. S²=Z, T²=S (not identity).*

---

**Q16.** A `ParameterVector('theta', 4)` creates:

A) 4 independent parameters named theta[0]..theta[3]  
B) One parameter named theta with value 4  
C) A 4-dimensional array parameter  
D) 4 qubits named theta  

*Answer: **A** — ParameterVector creates an indexed collection: theta[0], theta[1], ..., theta[3].*
...
C) The fastest backend available  
D) A local simulator with 5 qubits  

*Answer: **B** — `least_busy()` selects operational backend with fewest pending jobs.*
...
A) Finite differences: [f(x+ε)−f(x)]/ε  
B) Evaluating circuit at θ+π/2 and θ−π/2  
C) Automatic differentiation through the gate matrix  
D) Approximating the Jacobian numerically  

*Answer: **B** — Parameter shift: grad = [f(θ+π/2) − f(θ−π/2)] / 2. Exact, not approximate.*
...
C) Qubits 2,3 of the base circuit to qubits 0,1 of other  
D) Both A and C  
...
**You are ready. Study this guide, run every code block, and complete all IBM Quantum Learning paths. Register at [ibm.com/training/certification/C1000112](https://www.ibm.com/training/certification/C1000112) — good luck!**

---

*Cross-reference: [IBM Developer Cert Guide](./IBM_Developer_Quantum_CertGuide.md) (builds on this guide) · [Quantum AI Zero to Mastery](./zero-to-mastery.md) (full 12-week programme)*
