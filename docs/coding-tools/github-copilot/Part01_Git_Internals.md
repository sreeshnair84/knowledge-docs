...
I *Key design goals: speed, data integrity, support for distributed non-linear workflows, and explicit design for large projects.*
...
> I *Blobs are deduplicated automatically. If 1,000 files share the same content, Git stores only one blob.*
...
I *Always use annotated tags for releases — they carry a GPG signature, tagger identity, and timestamp. Lightweight tags are just named pointers and carry none of this.*
...
I *SHA-256 repositories are not backwards-compatible with SHA-1. Interoperability requires translation layers. At time of writing, most enterprise tooling still defaults to SHA-1.*
...
I *git gc --prune=now permanently deletes unreachable objects. Always check reflog before aggressive GC on production repositories.*
...
I *NEVER rebase commits already pushed to a shared branch. It rewrites history (new SHAs) causing divergence for all other developers. The golden rule: never rebase public history.*
...
I *git reset --hard is destructive. Use git revert for commits already on shared/protected branches.*
...
I *For very large repos, git maintenance is superior to git gc. It runs incrementally in background without blocking other operations.*
...
I *Server-side hooks (pre-receive, update, post-receive) enforce policy at the Git server level and cannot be bypassed by client-side hook manipulation. GitHub implements its own server-side hooks for branch protection rules.*
...
I *GitHub's infrastructure supports partial clone via the protocol v2 with filter capabilities. This is how GitHub.com itself handles git clone for very large repos like the VS Code repository.*
...
I *GitHub LFS storage and bandwidth are billed separately. Large orgs with many binaries should consider dedicated artifact stores (Artifactory, AWS S3 + CloudFront) and use LFS only for files that must be version-controlled alongside code.*
