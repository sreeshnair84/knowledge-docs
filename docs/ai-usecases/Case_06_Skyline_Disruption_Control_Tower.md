...
| Character | Role |
| ----------- | ------ |
| **Sofia Marchetti** ⭐ | Enterprise AI Architect, Operations Systems (EA) — our protagonist |
| **Derek Wallace** | VP Operations Control Centre (OCC) — sponsor |
...
*Draft v0.2 | Skyline Continental Airlines*
...
- ✅ Clear architectural separation established: agent for speed and synthesis, deterministic solver for legal/safety-critical calculation, human controller for execution decisions

- ✅ Permanent human execution authority boundary set at Incubation, framed as a regulatory and safety line, not a temporary trust threshold

- ✅ Cross-domain coordination (crew, aircraft, customer recovery) identified as the core synthesis challenge for the agentic layer
...
- ✅ Customer-impact estimation elevated to a first-class ranking dimension alongside operational and crew-legality cost, co-defined with Customer Recovery

- ✅ Explicit failure-mode reporting designed for solver time-outs, avoiding both silent failure and misleadingly complete partial output

- ✅ Human execution authority reaffirmed as absolute and permanent, framed by Regulatory & Safety as a design principle rather than a trust-earning milestone
...
*v1.0 | Approved 20 September 2026*
...
- ✅ General principle reinforced across the whole programme portfolio: model confidence can reflect data availability rather than ground truth, requiring explicit separation and disclosure

- ✅ Uncertainty-penalty mechanism built into ranking presentation for low-data-coverage cases across all three ranking dimensions

- ✅ Audit extended proactively to check for similar coverage imbalances in crew and aircraft-routing dimensions, finding and fixing a second, smaller instance
...
**Live Shadow-Mode Incident**

*Attendees: Sofia (EA, remote), Miguel (Lead Engineer, remote), Duty Controller Team*
:::

*A rapidly-developing ice storm forces ground stops at three major hub airports simultaneously. The disruption control tower is running in shadow mode — generating and ranking options in parallel with human planners, not yet authorised for controller use — during a genuine live event for the first time.*
...
*The following week, Miguel’s team traces the gap.*
...
- ✅ Root cause traced to historical-pattern-based generation under-exploring structurally valid but historically rare option types

- ✅ Systematic rules-based generation sweep added as a floor alongside pattern-based generation, addressing the gap at its source

- ✅ Shadow-mode live-event validation established as a required pre-go-live gate, not a formality — caught a real gap that synthetic evaluation had not
...
- ✅ A real controller case demonstrated the equal-prominence ranking design successfully counteracting automation-following instinct under genuine time pressure

- ✅ Confidence-flagging mechanism from Design stage directly influenced a real operational decision, validating the fix in production, not just in testing

- ✅ Zero incidents of controller execution authority being bypassed; human decision boundary held throughout first operating quarter
...
- ✅ Board-level pressure to extend autonomous execution, even to nominally low-severity cases, declined pending a full year of production track record and independent governance review

- ✅ Principle reaffirmed across the programme: risk-tier success does not transfer authorisation to a different tier automatically

- ✅ Human execution authority boundary sustained fourteen months into the programme despite strong performance results creating organisational pressure to relax it

- ✅ Programme closes its first operating year having reduced recovery decision time by 44% while holding every safety and authority boundary set at Incubation
...
*“The system that gives a controller better options faster is valuable. The system that quietly makes it easier for a tired controller to stop comparing options is dangerous — and they can look identical from a dashboard.”*
